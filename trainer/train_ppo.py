...
from model.MiniMindModel import MiniMindConfig, MiniMindForCausalLM  # MiniMind模型
...
def ppo_train_epoch(
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        response_ids = gen_out[:, input_len:]

        # 计算每条 response 的真实长度：遇到 eos 就停止，否则取最大生成长度
        is_eos = response_ids.eq(tokenizer.eos_token_id)
        ...
        response_lens = torch.where(
            has_eos,
            eos_pos + 1,
            torch.full_like(eos_pos, response_ids.size(1)),
        )

        # response mask: [B, R]
        resp_pos = torch.arange(response_ids.size(1), device=gen_out.device).unsqueeze(0)
        response_mask = resp_pos < response_lens.unsqueeze(1)

        # 解码 response
        responses_text = [
            tokenizer.decode(response_ids[i, : response_lens[i]], skip_special_tokens=True)
        ...
        # full attention mask: prompt mask + response mask
        full_mask = torch.cat(
            [enc.attention_mask, response_mask.long()],
            dim=1
        )
        ...
        last_indices = input_len + response_lens - 1
        batch_idx = torch.arange(gen_out.size(0), device=gen_out.device)

        values = value_seq[batch_idx, last_indices]
        ...
        # response 在 gen_out 中从 input_len 开始
        # 但 labels/logp_tokens shift 了一位，所以起点是 input_len - 1
        resp_start = input_len - 1

        final_mask = torch.zeros_like(logp_tokens, dtype=torch.bool)
        final_mask[:, resp_start:] = response_mask[:, : logp_tokens.size(1) - resp_start]
