...
def grpo_train_epoch(
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # ============================================================
        # 小改 1：提前构造 completion_mask
        # 后面既用于 full_attention_mask，也用于最终 loss
        # ============================================================
        ...
        # ============================================================
        # 小改 2：构造完整 attention_mask
        # prompt_inputs["attention_mask"] 原本是 [batch_size, prompt_len]
        # generate 后 outputs 是 [batch_size * num_generations, prompt_len + completion_len]
        # 所以 prompt mask 需要 repeat_interleave
        # ============================================================
        prompt_attention_mask = prompt_inputs["attention_mask"].repeat_interleave(
            args.num_generations,
            dim=0,
        )

        full_attention_mask = torch.cat(
            [prompt_attention_mask, completion_mask],
            dim=1,
        )


        # ============================================================
        # 小改 3：get_per_token_logps 多传一个 attention_mask
        # 其余逻辑尽量保持你原来的版本
        # ============================================================
        def get_per_token_logps(mdl, input_ids, attention_mask, n_keep):
            input_ids = (
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="MiniMind GRPO (Group Relative Policy Optimization)"
    )
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="MiniMind-GRPO", help="wandb项目名"
    )
